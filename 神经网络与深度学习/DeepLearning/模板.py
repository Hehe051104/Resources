...
best_path = "houseprices/房价预测.pt"  # 保存最佳模型参数的路径
...
def main():
    ...
    df=pd.read_csv('houseprices/house prices(1).csv', encoding='gbk')
    ...
    dataset = TensorDataset(X, y)  #Dataset 提供“索引式存储”
    ...
    #scheduler  简单:OneCycleLR 复杂:CosineAnnealingLR
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=10)     # 学习率调度器: 余弦退火, T_max=10 表示10个epoch衰减到接近0,自适应学习率,在每轮训练或测试后step调用
    ...
    for i in range(epoch):
        ...
        print("整体测试集上的Loss: {:.4f}".format(avg_test_loss))
